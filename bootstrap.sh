...
clone_or_pull() {
  ...
  if [ -d "${target_dir}/.git" ]; then
    echo "[INFO] Repo ${repo_name} déjà présent dans ${target_dir}, mise à jour..."
...
echo "[INFO] Clonage / mise à jour des repos..."
clone_or_pull "${DISCORD_REPO_URL}" "${DISCORD_DIR}" "discord"
clone_or_pull "${GPT_REPO_URL}" "${GPT_DIR}" "gpt"
clone_or_pull "${GEMINI_REPO_URL}" "${GEMINI_DIR}" "gemini"
